/**
 * Frontend Interview Framework — Excel Generator
 * Node.js built-in modules only. No npm dependencies required.
 *
 * Run: node generate-excel.js
 * Output: Frontend-Interview-Framework.xlsx
 */
...
// Verified accessible URLs (does not require login to view)
// ─────────────────────────────────────────────────────────────────────────────

const figmaResources = [
  {
    name: 'Material 3 Design Kit (Google)',
    url: 'https://www.figma.com/community/file/1035203688168086460',
    covers: 'Table, Button, Badge, Form controls, Modal, Pagination. Most complete free kit.',
    note: 'Open in Figma. Free to duplicate. Go to the "Components" page, find the Data Table section.',
    groups: '1, 2, 3, 4, 9, 10'
  },
  {
    name: 'Figma Community search. Data Table UI',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=data+table',
    covers: 'Browse multiple free table designs. Sort by "Most liked" to find highest quality.',
    note: 'Search results page. Pick any free file to duplicate.',
    groups: '1, 2, 3, 4'
  },
  {
    name: 'Figma Community search. Admin Dashboard',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=admin+dashboard+table',
    covers: 'Full admin dashboard designs with table, filter bar, pagination, status badges, bulk actions.',
    note: 'Look for files with "Free" label. Most have complete table UI with all states.',
    groups: '1, 2, 3, 4, 5'
  },
  {
    name: 'Figma Community search. Loading skeleton states',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=skeleton+loading+empty+state',
    covers: 'Skeleton loading rows, empty state illustrations, error state designs.',
    note: 'Use for Group 1 questions about loading, empty and error states.',
    groups: '1, 6'
  },
  {
    name: 'Figma Community search. Filter and Search bar',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=filter+bar+search+form',
    covers: 'Filter bar designs, multi-select dropdowns, date range pickers, search inputs.',
    note: 'Combine with your table frame to build the complete interview scenario.',
    groups: '2, 10'
  },
  {
    name: 'Figma Community search. Pagination component',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=pagination+component',
    covers: 'Pagination bars in all variants. Page numbers, items-per-page, infinite scroll indicators.',
    note: 'Use for Group 3 questions about pagination, infinite scroll strategy.',
    groups: '3'
  },
  {
    name: 'Figma Community search. Bulk actions selection',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=bulk+action+table+selection',
    covers: 'Checkbox column, indeterminate state, bulk action toolbar, row selection states.',
    note: 'Use for Group 4 questions about multi-select and bulk operations.',
    groups: '4'
  },
  {
    name: 'Figma Community search. Accessibility annotations',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=accessibility+annotation+kit',
    covers: 'ARIA annotation stickers, focus order overlays, screen reader annotations.',
    note: 'Overlay on your table to show which ARIA attributes go where.',
    groups: '9'
  },
  {
    name: 'Figma Community search. Architecture diagram',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=architecture+diagram+kit+frontend',
    covers: 'System design boxes, arrows, micro-frontend diagrams, module federation flow.',
    note: 'Use for Group 8 micro-frontend questions to draw Shell and Remote relationship.',
    groups: '8'
  },
  {
    name: 'Figma Community search. Inline edit table row',
    url: 'https://www.figma.com/community/search?resource_type=file&sort_by=relevancy&query=inline+edit+table+row',
    covers: 'Table row in default state vs edit mode. Save and cancel buttons, input fields inside cells.',
    note: 'Use for Group 4 inline editing and Group 10 form-in-table questions.',
    groups: '4, 10'
  },
  {
    name: 'Figma Quick Start Tip. Build your own table',
    url: 'https://help.figma.com/hc/en-us/articles/360040314193-Guide-to-components-in-Figma',
    covers: 'Official Figma guide for creating components. Follow this to build a reusable table row component.',
    note: 'Steps. Create a Frame (F key). Add a rectangle for header row. Add text for column names. Group as Component (Ctrl+Alt+K).',
    groups: 'All'
  },
  {
    name: 'Alternative. FigJam (free, no design skills needed)',
    url: 'https://www.figma.com/figjam/',
    covers: 'Whiteboard tool. Drag in screenshots from the internet, add sticky notes for each question, draw arrows for flow.',
    note: 'Recommended if you have no Figma design experience. Screenshot a table from any admin UI (e.g. GitHub issues list) and paste it. Add sticky notes per question group.',
    groups: 'All'
  }
];
...
// ─────────────────────────────────────────────────────────────────────────────
// INTERVIEW Q&A DATA
// Formatting rules applied throughout all text fields.
// No single quotes around terms. Use double quotes instead.
// No dash as separator. Use period or comma instead.
...
function sheet(rows, frozenRow = 1) {
  const widths = rows[0].map((_, ci) =>
    Math.min(80, Math.max(12, ...rows.map(r => String(r[ci] || '').length * 0.9)))
  );
  ...
  x += `<sheetViews><sheetView tabSelected="0" workbookViewId="0">`;
  x += `<pane ySplit="${frozenRow}" topLeftCell="A${frozenRow+1}" activePane="bottomLeft" state="frozen"/>`;
  x += `</sheetView></sheetViews>`;
  x += `<sheetFormatPr defaultRowHeight="15"/>`;
  x += `<cols>` + widths.map((w,i) =>
    `<col min="${i+1}" max="${i+1}" width="${w}" customWidth="1"/>`).join('') + `</cols>`;
  x += `<sheetData>`;
  rows.forEach((row, ri) => {
    const rn = ri + 1;
    const isHdr = ri === 0;
    const ht = isHdr ? 22 : 90;
    x += `<row r="${rn}" ht="${ht}" customHeight="1">`;
    row.forEach((cell, ci) => {
      const ref = `${COL[ci]}${rn}`;
      const s = isHdr ? 1 : (typeof cell === 'number' ? 3 : 2);
      x += `<c r="${ref}" t="inlineStr" s="${s}"><is><t>${esc(cell)}</t></is></c>`;
    });
    x += `</row>`;
  });
  x += `</sheetData><pageSetup orientation="landscape"/></worksheet>`;
  return x;
}
...
<fills count="5">
  ...
  <fill><patternFill patternType="solid"><fgColor rgb="FFE8EFF7"/></patternFill></fill>
</fills>
<borders count="2">
  <border><left/><right/><top/><bottom/><diagonal/></border>
  <border>
    <left style="thin"><color rgb="FFCCD6E3"/></left>
    <right style="thin"><color rgb="FFCCD6E3"/></right>
    <top style="thin"><color rgb="FFCCD6E3"/></top>
    <bottom style="thin"><color rgb="FFCCD6E3"/></bottom>
  </border>
</borders>
...
<sheets>
  ...
  <sheet name="All Questions and Answers" sheetId="2" r:id="rId2"/>
  <sheet name="Figma and Design Resources" sheetId="3" r:id="rId3"/>
</sheets>
</workbook>`;
...
// Sheet 1. Overview
const overviewRows = [
  ['Group', 'Group Name', 'Target Level', 'Question Count', 'Figma Resource Available', 'Key Topics Covered']
];
const topicMap = [
...
groups.forEach((g, i) => {
  overviewRows.push([g.id, g.name, g.level, g.questions.length, 'Yes. See Figma sheet', topicMap[i]]);
});
overviewRows.push(['', '', '', `Total. ${groups.reduce((s, g) => s + g.questions.length, 0)} questions`, '', '']);
...
const figmaRows = [
  ['Resource Name', 'Direct URL', 'What It Covers', 'How to Use It', 'Relevant Groups']
];
figmaResources.forEach(f => {
  figmaRows.push([f.name, f.url, f.covers, f.note, f.groups]);
});

// ─── Write files ─────────────────────────────────────────────────────────────

const OUT = path.join(__dirname, 'xlsx-source');
const dirs = [OUT, `${OUT}/xl`, `${OUT}/xl/worksheets`, `${OUT}/_rels`, `${OUT}/xl/_rels`];
dirs.forEach(d => fs.mkdirSync(d, { recursive: true }));

fs.writeFileSync(`${OUT}/[Content_Types].xml`, CT);
fs.writeFileSync(`${OUT}/_rels/.rels`, RELS);
fs.writeFileSync(`${OUT}/xl/workbook.xml`, WB);
fs.writeFileSync(`${OUT}/xl/styles.xml`, STYLES);
fs.writeFileSync(`${OUT}/xl/_rels/workbook.xml.rels`, WB_RELS);
fs.writeFileSync(`${OUT}/xl/worksheets/sheet1.xml`, sheet(overviewRows));
fs.writeFileSync(`${OUT}/xl/worksheets/sheet2.xml`, sheet(qaRows));
fs.writeFileSync(`${OUT}/xl/worksheets/sheet3.xml`, sheet(figmaRows));
...
`;
fs.writeFileSync(path.join(__dirname, 'zip.ps1'), ps);

const total = groups.reduce((s, g) => s + g.questions.length, 0);
console.log(`XML files written. Total questions. ${total}`);
console.log(`Run zip.ps1 to produce the .xlsx file.`);
